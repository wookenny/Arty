...
real Ray::intersectBox(const Vector3 &boxMin, const Vector3 &boxMax, const Vector3 &pos, bool debug) const{
	...
	tmin =  (bounds[_sign[0]]   [0]  -  pos[0]) * _inv_direction[0];
	tmax =  (bounds[1-_sign[0]] [0]  -  pos[0]) * _inv_direction[0];
	tymin = (bounds[_sign[1]]   [1]  -  pos[1]) * _inv_direction[1];
	tymax = (bounds[1-_sign[1]] [1]  -  pos[1]) * _inv_direction[1];
	if(debug){
	...
	tzmin = (bounds[_sign[2]]  [2]   - pos[2]) * _inv_direction[2];
	tzmax = (bounds[1-_sign[2]][2]   - pos[2]) * _inv_direction[2];
	if(debug){
